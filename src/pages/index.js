...
export default function Home() {
  // const [navTitles,setNavTitles]=useState(navTitle)
...
let subNav={
 ...
 let mostViewedData={
  ...
  return (
    <Layout subNav={subNav} breadcrumbTitle={mostViewedData.m4} trendingData={trendingData} headerData={headerData} footerDta={footerDta} >
      <Banner bannerdata={bannerdata} />
      <Trending  trendingData={trendingData}  headerData={headerData}/>
       <BrandLogo brandData={brandData}/>
      <Discount  disData={disData}/>
      <Advantage  adData={adData}/>
      {/* <LatestNews /> */}
       <Subscribe subData={subData} />
    </Layout>
  );
}
...
// const data= await fetch('https://staging.xprestores.com/api/category/list')
// const res=await data.json()
// let navTitle={}
// let subNav=[]
// let navCategory=res.results.map((result)=>{
//   // console.log("main category",result.title);
// let category={
//   name: result.title,
//   path: `/${result.title}`,
//   subNav:result.has_subcategories?(
//     result.subcategories.map(sub=>{
//       return (subNav=[...subNav,{name:sub.title,path:`/${sub.title}`,subNav:"sub sub sub"}])
//     //   subNav=[...subNav,
//     //   {name:sub.title,
//     //    path: `/${sub.title}`,
//     // }]
...
//   return {
//     props: {navTitle}, // will be passed to the page component as props
//   }
// }
